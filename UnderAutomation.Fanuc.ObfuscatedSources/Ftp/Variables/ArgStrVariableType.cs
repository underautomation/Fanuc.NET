//              WARNING
// This file is an empty shell containing only public C# items.
// The internal code is hidden; to access it, you need to obtain a Source licence of the library.


namespace Ftp.Variables {
	/// <summary>
	/// Describes the Fanuc type ARG_STR_T
	/// </summary>
	public class ArgStrVariableType : GenericVariableType, IGenericVariableType {


		public ArgStrVariableType()
		{
			// Source is hidden, a Source licence is needed to access internal code...
		}

		/// <summary>
		/// Value of variable $TITLE
		/// </summary>
		public string Title { get; }

		/// <summary>
		/// Value of variable $ITEM1
		/// </summary>
		public string Item1 { get; }

		/// <summary>
		/// Value of variable $ITEM2
		/// </summary>
		public string Item2 { get; }

		/// <summary>
		/// Value of variable $ITEM3
		/// </summary>
		public string Item3 { get; }

		/// <summary>
		/// Value of variable $ITEM4
		/// </summary>
		public string Item4 { get; }

		/// <summary>
		/// Value of variable $ITEM5
		/// </summary>
		public string Item5 { get; }

		/// <summary>
		/// Value of variable $ITEM6
		/// </summary>
		public string Item6 { get; }

		/// <summary>
		/// Value of variable $ITEM7
		/// </summary>
		public string Item7 { get; }

		/// <summary>
		/// Value of variable $ITEM8
		/// </summary>
		public string Item8 { get; }

		/// <summary>
		/// Value of variable $ITEM9
		/// </summary>
		public string Item9 { get; }

		/// <summary>
		/// Value of variable $ITEM10
		/// </summary>
		public string Item10 { get; }

		/// <summary>
		/// Value of variable $ITEM11
		/// </summary>
		public string Item11 { get; }

		/// <summary>
		/// Value of variable $ITEM12
		/// </summary>
		public string Item12 { get; }

		/// <summary>
		/// Value of variable $ITEM13
		/// </summary>
		public string Item13 { get; }

		/// <summary>
		/// Value of variable $ITEM14
		/// </summary>
		public string Item14 { get; }

		/// <summary>
		/// Value of variable $ITEM15
		/// </summary>
		public string Item15 { get; }

		/// <summary>
		/// Value of variable $ITEM16
		/// </summary>
		public string Item16 { get; }

		/// <summary>
		/// Value of variable $ITEM17
		/// </summary>
		public string Item17 { get; }

		/// <summary>
		/// Value of variable $ITEM18
		/// </summary>
		public string Item18 { get; }

		/// <summary>
		/// Value of variable $ITEM19
		/// </summary>
		public string Item19 { get; }

		/// <summary>
		/// Value of variable $ITEM20
		/// </summary>
		public string Item20 { get; }

		/// <summary>
		/// Type Name on the robot
		/// </summary>
		public override string FanucInternalTypeName => default;
	}
}
