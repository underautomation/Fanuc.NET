...
public partial class SftpControl : UserControl, IUserControl
{
    ...
    public SftpControl(FanucRobot robot)
    {
        _robot = robot;
        InitializeComponent();
    }
    ...
    private void FillList(string path)
    {
        ...
        var files = _robot.Ftp.GetListing(path);
    ...
    private void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        foreach (var itm in lstFolder.SelectedItems.OfType<ListViewItem>())
        {
            ...
            {
                _robot.Ftp.DeleteFile(file.FullName);
            }
            else
            {
                _robot.Ftp.DeleteDirectory(file.FullName);
            }
            Thread.Sleep(500);
            ReloadList();
        }
    ...
    private void btnUpload_Click(object sender, EventArgs e)
    {
        ...
        {
            _robot.Ftp.Upload(selectedFile, GetPath() + Path.GetFileName(dlgOpen.FileName).Replace(@"\", "/"));
        }
    ...
    private void btnDownload_Click(object sender, EventArgs e)
    {
        ...
        _robot.Ftp.DownloadFile(dlgSave.FileName, file.FullName);
    ...
    private void lstFolder_AfterLabelEdit(object sender, LabelEditEventArgs e)
    {
        ...
        try
        {
            if (lstFolder.Items[e.Item].Tag is null)
            {
                ...
                _robot.Ftp.CreateDirectory(txtPath.Text + e.Label);
            }
        }
        finally
        {
